...
result=$(xcodebuild archive -${Project} ${project_name}.${Xcodeproj} -configuration ${configuration} -scheme ${project_name} -archivePath ${export_directory}${project_name}.xcarchive -allowProvisioningUpdate CODE_SIGN_STYLE="Automatic" DEVELOPMENT_TEAM="${DEVELOPMENT_TEAM}" > ${export_directory}buildLog.txt)
